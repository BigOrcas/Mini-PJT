	...
	function fncGetList(currentPage) {
	   ...
	   console.log(currentPage);
		...
		$( "td.cancel" ).on("click" , function() {
			...
			self.location ="/purchase/cancel?tranNo="+tranNo+"&tranCode="+tranCode;
		});
...
<body>
	...
	<c:forEach var="purchase" items="${list}">
		...
		<td align="center">현재 구매완료 상태 입니다.</td>
		</c:when>
		<c:when test="${purchase.tranCode=='200' }">
		<td align="center">현재 배송중입니다.</td>
		</c:when>
		<c:otherwise>
		<td align="center">배송 완료되었습니다.</td>
		</c:otherwise>
		</c:choose>
		...
		<c:choose>
		<c:when test="${purchase.tranCode=='200'}">
		<td align="right" class="divy" value1="${purchase.tranNo }" value2="${purchase.tranCode}">물건도착</td>	
		</c:when>
		<c:when test="${purchase.tranCode=='100'}">
		<td align="right" class="cancel" value1="${purchase.tranNo }" value2="${purchase.tranCode}">구매취소</td>	
		</c:when>
		<c:when test="${purchase.tranCode=='000'}">
	...
	<tr>
		...
		<td id="${purchase.tranNo}" colspan="11" bgcolor="#ffffff" height="1">
